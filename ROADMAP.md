...
| ✅ robowire battery/charging safety checks: **E43** (charge controller's declared chemistry/cell-count matches the battery it charges), **E44** (multi-cell pack declares `has_bms`, warn), **E45** (fuse/PTC reachable from battery positive via the worst-case bridge graph, warn) — `roboparts::SourceDecl` gained `chemistry`/`cell_count`/`has_bms`, `Part` gained `charge_profile`; the numbered curriculum's reference battery/switch path got a protection-board variant (`lipo-2s-260-bms`/`lipo-3s-450-bms`) and an inline fuse so stages 1-7 stay warn-free, plus a new standalone **stage 8** (`08-battery-protection`) whose broken variant is the first lesson to demonstrate E44+E45 directly (bare pack + no fuse, one mistake, two warns); the bare `lipo-2s-260` + fuseless solar-charging demo stay as the worked example of the exact gap these checks catch | robowire (informal, between M1 and M2) | **done:** `cargo test` green across all seven suites; new lesson-e43 fixture + numbered stage 8 + hand-written E44/E45 warn tests |
